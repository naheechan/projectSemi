...
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/css/wordingBoardDetail.css">
<%
	...
	<div class="wordingDetail-container">
		<div id="wordingDetail">
			...
			<div class="wordingDetailImg">
				<!-- 글귀 사진 -->
				<img
					src="<%=request.getContextPath()%>/image/wordingBoard/<%=pic.getWordingPicName()%>">
				<div class="img-text">
					<p><%=w.getWordingContent()%></p>
				</div>
			</div>
			...
			<div class="wordingDetailWriterPic">
				<!--작성자 프로필사진(동그라미)-->
				<img
					src="<%=request.getContextPath()%>/image/wordingBoard/profileSample.jpg">
			</div>
		...
		<div class="editDiv">
			<%
				if (logginedMember != null && ("admin".equals(logginedMember.getMemberId())
		...
		<div class="wordingDetail2">
			<div class="wordingViewCnt">
				<span>조회수&nbsp;&nbsp;</span> <span><%=w.getWordingViewCount()%></span>
			</div>

			<div class="wordingDate">
				<span>작성일&nbsp;&nbsp;</span> <span><%=w.getWordingDate()%></span>
			</div>
		</div>
		...
		<div id="wordingLike">
			...
			<div class="likeImg">
				<img
					src="<%=request.getContextPath()%>/image/wordingBoard/<%=(Integer) request.getAttribute("likeChk") == 0 ? "unlike.png" : "like.png"%>">
			</div>
			<!-- 좋아요 개수 -->
				...
				<div class="writeComments">
					<table>
						<tr>
							<td>
								<div>
									<textarea cols="100" rows="5" id="comContent" name="comContent"></textarea>
								</div>
							</td>
							<td>
								<div>
									<button class="btn writeComBtn">등록</button>
								</div>
							</td>
						</tr>
					</table>
					...
					wordingNo:"<%=w.getWordingNo()%>",
										...
										success : function(data) {
											...
											if (data != null) {
												...
												re.find(".commentWriter").html(
														data["writer"]);
												re.find(".commentDate").html(
														data["comDate"]);
												re
														.find(".commentContent")
														.html(
																data["comContent"]);
												if (parentCom.nextAll(
														".level1Comment")
														.first().length) {
													parentCom.nextAll(
															".level1Comment")
